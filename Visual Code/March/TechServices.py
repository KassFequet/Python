# Description: Tech Services Company - Lesson 25/26 - Question 5 (Missed Class) - "Process a receipt for customer purchases from Tech Services Inc.""
# Author: Mo and Class
# Date(s): March 13/25 - March 14/25
...
while True:
    ...
    # Display results
    print()
    print(f"Tech Services In.                            Invoice No:       {INV_NUM:>5d}")
    print(f"Customer Invoice                             Invoice Date: {FV.FDateM(CUR_DATE)}")
    #Short date - YYY-MM-DD, Medium Date is dd-MON-yy, Long Date is Day, Month, dd, yyyy
    ...
    #Use the simpliest form of a file - comma seperated text file.
    #Each line (record) will be stored with comma seperators. For example:
    #   Value, Value, Value, Value, Value...(Keeps as many as we need to keep track of)
    #Each record will contain the same number of fields (Values) in the same order.
    
    #To add a data to a file
